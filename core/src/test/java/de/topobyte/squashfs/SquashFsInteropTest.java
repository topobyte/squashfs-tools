...
public class SquashFsInteropTest
{
	...
	public static Collection<Object[]> data()
	{
		return Arrays.asList(new Object[][] {
				...
				{ "mapped-with-cache",
						(ReaderCreator) (a -> createMappedReaderWithCache(
								a)) } });
	}
	...
	public void emptyArchiveShouldWork() throws Exception
	{
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertEquals("wrong permissions", (short) 0700,
					root.getPermissions());
	...
	public void archiveWithDirectoriesAtRootShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			for (int i = 0; i < 10; i++) {
				SquashFsEntry entry = writer.entry(String.format("/dir-%d", i))
						...
						.uid(0).gid(0).permissions((short) 0755).build();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			// verify links
			List<DirectoryEntry> children = reader
					.getChildren(reader.getRootInode());
			...
			for (int i = 0; i < 10; i++) {
				...
				assertEquals(String.format("wrong name for entry %d", i),
						String.format("dir-%d", i), child.getNameAsString());
	...
	public void archiveWithSyntheticDirectoryShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/dir").directory().synthetic().uid(0).gid(0)
					.permissions((short) 0755).build();

			writer.entry("/dir").directory().uid(0).gid(0)
					.permissions((short) 0711).build();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.BASIC_DIRECTORY,
					dir.getInodeType());
			assertEquals("wrong permissions", (short) 0711,
					dir.getPermissions());
		}
	}

	@Test
	public void archiveWithSyntheticDirectoryInReverseShouldWork()
			throws Exception
	{
		File archive = temp.newFile();
	...
	public void archiveWithFilesAtRootShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			for (int i = 0; i < 10; i++) {
				writer.entry(String.format("/file-%d", i))
						.lastModified(System.currentTimeMillis()).uid(0).gid(0)
						.content(new ByteArrayInputStream(
								String.format("file-%d", i)
										.getBytes(StandardCharsets.ISO_8859_1)))
						.permissions((short) 0644).build();
			}
			writer.finish();
		}
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			for (int i = 0; i < 10; i++) {
				INode file = reader
						.findInodeByPath(String.format("/file-%d", i));
				assertSame(String.format("Wrong type for entry %d", i),
	...
	public void archiveWithFileContainingFullBlockShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			SquashFsEntry entry = writer.entry("/full.dat")
					.lastModified(Instant.now()).uid(1000).gid(2000)
			...
			assertEquals("wrong data block count", 1,
					entry.getDataBlocks().size());
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertEquals("wrong size", 131072L,
					((FileINode) file).getFileSize());
			...
			// test writing entire file
			{
				...
				INodeRef ref = reader.getExportTable()
						.getInodeRef(file.getInodeNumber());
				INode file2 = reader.findInodeByInodeRef(ref);
				assertEquals("wrong inode number using write()",
						file.getInodeNumber(), file2.getInodeNumber());
			}
			...
			// test reading data
			{
				...
				try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
					int c;
					long fileOffset = 0L;
					while ((c = reader.read(file, fileOffset, xfer, 0,
							xfer.length)) >= 0) {
						if (c >= 0) {
							bos.write(xfer, 0, c);
							fileOffset += c;
				...
				INodeRef ref = reader.getExportTable()
						.getInodeRef(file.getInodeNumber());
				INode file2 = reader.findInodeByInodeRef(ref);
				assertEquals("wrong inode number using read()",
						file.getInodeNumber(), file2.getInodeNumber());
			}
		}
	}
			...
			throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/sparse.dat").lastModified(new Date()).uid(0).gid(0)
					.content(new ByteArrayInputStream(content))
					.permissions((short) 0644).build();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.EXTENDED_FILE,
					file.getInodeType());
			assertEquals("wrong sparse count", 131072L,
					((FileINode) file).getSparse());
			assertEquals("wrong size", 262144L,
					((FileINode) file).getFileSize());
	...
	public void archiveWithHardLinkToFileShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			...
			SquashFsEntry target = writer.entry("/target.dat")
					.lastModified(System.currentTimeMillis()).uid(1).gid(1)
					.content(new ByteArrayInputStream(content))
					.permissions((short) 0644).build();

			// write second file
			SquashFsEntry source = writer.entry("/source.dat")
					.hardlink("/target.dat").build();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertEquals("wrong size", 1024L,
					((FileINode) file2).getFileSize());
			try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
				reader.writeFileStream(file, bos);
				assertArrayEquals(content, bos.toByteArray());
			}
			...
			throws Exception
	{
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.BASIC_FILE, file.getInodeType());
			assertEquals("wrong size", 275000L,
					((FileINode) file).getFileSize());
	...
	public void archiveWithCharDeviceShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/dev").directory().uid(0).gid(0)
					.permissions((short) 0755).build();

			SquashFsEntry dev = writer.entry("/dev/zero").uid(0).gid(0)
					.charDev(1, 5).permissions((short) 0644).build();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.BASIC_CHAR_DEVICE,
					dev.getInodeType());
	...
	public void archiveWithBlockDeviceShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			...
			SquashFsEntry dev = writer.entry("/dev/loop0").uid(0).gid(0)
					.blockDev(7, 0).permissions((short) 0644).build();
	...
	public void archiveWithFifoShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			...
			writer.entry("/dev/log").uid(0).gid(0).fifo()
					.permissions((short) 0666).build();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertEquals("wrong permissions", (short) 0666,
					dev.getPermissions());
		}
	}
	...
	public void archiveWithSymlinkShouldWork() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/target.dat").lastModified(System.currentTimeMillis())
					.uid(0).gid(0).content(new ByteArrayInputStream(content))
					.permissions((short) 0644).build();

			SquashFsEntry link = writer.entry("/link.dat").uid(0).gid(0)
					.symlink("/target.dat").permissions((short) 0644).build();

			writer.finish();

			assertEquals("wrong link target", "/target.dat",
					link.getSymlinkTarget());
		}
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.BASIC_SYMLINK,
					link.getInodeType());
	...
	public void fileWithNullNameShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry(null).lastModified(System.currentTimeMillis()).uid(0)
					.gid(0).content(new ByteArrayInputStream(new byte[0]))
					.permissions((short) 0644).build();
		}
	}
	...
	public void fileWithEmptyNameShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("").lastModified(System.currentTimeMillis()).uid(0)
					.gid(0).content(new ByteArrayInputStream(new byte[0]))
					.permissions((short) 0644).build();
		}
	}
	...
	public void fileWithNonAbsoluteNameShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("foo").lastModified(System.currentTimeMillis()).uid(0)
					.gid(0).content(new ByteArrayInputStream(new byte[0]))
					.permissions((short) 0644).build();
		}
	}
	...
	public void fileWithNameEndingInSlashShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/foo/").lastModified(System.currentTimeMillis())
					.uid(0).gid(0)
	...
	public void fileWithNoUidShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/no-uid.dat").lastModified(System.currentTimeMillis())
					.gid(0).content(new ByteArrayInputStream(new byte[0]))
					.permissions((short) 0644).build();
		}
	}
	...
	public void fileWithNoGidShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/no-gid.dat").lastModified(System.currentTimeMillis())
					.uid(0).content(new ByteArrayInputStream(new byte[0]))
					.permissions((short) 0644).build();
		}
	}
	...
	public void fileWithNoPermissionsShouldFail() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			writer.entry("/no-permissions.dat")
					.lastModified(System.currentTimeMillis()).uid(0).gid(0)
					.content(new ByteArrayInputStream(new byte[0])).build();
		}
	}
	...
	public void fileWithNoTimestampShouldUseDefault() throws Exception
	{
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			SquashFsEntry entry = writer.entry("/no-timestamp.dat").uid(0)
					.gid(0).content(new ByteArrayInputStream(new byte[0]))
					.permissions((short) 0644).build();

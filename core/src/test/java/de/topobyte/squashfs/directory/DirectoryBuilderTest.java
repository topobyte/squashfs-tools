...
import de.topobyte.squashfs.test.DirectoryTestUtils;
...
public class DirectoryBuilderTest
{
	...
	public void writeShouldSerializeData() throws Exception
	{
		...
		List<DirectoryElement> elements = DirectoryTestUtils
				.deserializeDirectory(data);
		...
		assertEquals("wrong offset", (short) 3, entry.offset);
		assertEquals("wrong inode number delta", (short) 0,
				entry.inodeNumberDelta);
		assertEquals("wrong type", INodeType.BASIC_FILE.value(), entry.type);
